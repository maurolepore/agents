...
Meta-repo for token-efficient, agent-invoked tooling.

## Layout

- `bin/` — executables (add `bin/` to `PATH`). Only directory with real code.
- `skills/` — `SKILL.md` per subdir; canonical store (ships with `find-skills` + example). Discovered globally via symlinks (see Setup).
...
```sh
export PATH="<path-to-agents>/bin:$PATH"  # required — wrappers not found otherwise
setup-agents                              # one-time: ensures bin/ on PATH for this shell + calls setup-opencode
...
`~/.config/opencode/skills`, `~/.agents/skills`, `~/.claude/skills` -> `<repo>/skills`;
`~/.config/opencode/commands` -> `<repo>/commands`. `setup-opencode` merges any pre-existing global dirs before linking (idempotent, `--dry-run` available). `setup-agents` additionally ensures `bin/` on `PATH` for the current shell before delegating. Restart opencode after running.
